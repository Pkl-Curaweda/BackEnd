...
const generatePDF = async () => {
  try {
    const datas = await printInvoice();
    const doc = new PDFDocument();
    const stream = fs.createWriteStream("hasil.pdf");

    // Menambahkan event listener untuk mengetahui kapan penulisan selesai
    stream.on("finish", () => {
      console.log("Invoice Data:", datas); // Menampilkan data di konsol setelah penulisan selesai
    });

    doc.pipe(stream);

    // Path ke file gambar yang akan ditambahkan
    //   const imagePath = "../../image/QR-Jessica30.png"; // Ubah path sesuai dengan lokasi gambar Anda
    ...
    doc.moveUp(1);
    doc.fontSize(11).text(`Guest Name: ${datas.guestName}.`, {
      align: "right",
    });
    doc.moveDown();
    ...
    doc
      ...
      .stroke("green");
    doc.moveDown();
    doc.fontSize(11).text(`Arrival: ${datas.arrivalDate}.`, {
      align: "left",
    });
    doc.moveDown();
    doc.fontSize(11).text(`Departure: ${datas.departureDate}.`, {
      align: "left",
    });
    doc.moveDown(2);
      ...
      datas: [
        {
          ...
          date: "bold:12/02/2023",
          description: "bold:Room rate",
          amount: "$1",
        },
        {
          options: { columnColor: "#D9D9D9" },
          date: "",
