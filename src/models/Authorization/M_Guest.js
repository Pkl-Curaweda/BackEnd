...
const {
  getAllRoomIdReservedByReserverId,
} = require("../Reservation/M_ResvRoom");

const CreateNewGuest = async (data) => {
  try {
    const salt = await bcrypt.genSalt();
    data.password = await bcrypt.hash(data.password, salt);
    const userExist = await guestClient.findUnique({
...
const GuestLogin = async (method, data) => {
  try {
    ...
    const reservedRoom = await getAllRoomIdReservedByReserverId(
      guest.Reserver.id
    );
...
const GenerateGuestQrCode = async (guestData) => {
  ...
  if (!fs.existsSync(path)) {
    ...
    const storedQR =
      "http://localhost:3000/auth/guest/login/qr?encryptedData=" +
      encryptedData;
...
const GetAllGuests = async () => {
  try {
    const guests = await guestClient.findMany({
      select: { username: true, name: true, contact: true },
    });
    return guests;
  } catch (err) {
    ThrowError(err);
